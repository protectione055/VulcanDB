...
class Stage {
 ...
 private:
  ...
  int64_t event_ref_;                    // # of outstanding events
  Threadpool *th_pool_ = nullptr;        // Threadpool for this stage
};

inline void Stage::set_pool(Threadpool *th) {
  ASSERT((th != NULL), "threadpool not available for stage {}",
         this->get_name());
  ASSERT(!connected_, "attempt to set threadpool while connected: {}",
         this->get_name());
  th_pool_ = th;
}

inline void Stage::push_stage(Stage *st) {
  ASSERT((st != NULL), "next stage not available for stage {}",
         this->get_name());
  ASSERT(!connected_, "attempt to set push stage while connected: {}",
         this->get_name());
